...
if [[ ! -e "$CURRENT_THEME" ]]; then
    echo "Error: $CURRENT_THEME does not exist."
    exit 1
fi
...
use_current_theme() {
    ...
    accent="$color5"
    notify-send -h string:x-canonical-private-synchronous:sys-notify-dtheme -u normal -i ${PATH_MAKO}/icons/palette.png "Applying Current Theme..."
}
